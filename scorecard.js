...
function cb(err,response,html){
   if(err){
       ...
       extractPlayerDeatils(html);
   }
}

function extractPlayerDeatils(html){
    let selTool=cheerio.load(html);
    let detailsElem= selTool(".match-info.match-info-MATCH .description");
    ...
    for(let i=0;i<nameOfTeams.length;i++){
        let allRowsOfTeam=selTool(batsmanTableOfTeams[i]).find("tbody tr");
        for(let j=0;j<allRowsOfTeam.length;j++){
            let allcols=selTool(allRowsOfTeam[j]).find("td");
            if(allcols.length==8){
               ...
               console.log(`teamName ${myTeamName} playerName ${name} venue ${venue} Date ${date} opponent ${opponentTeamName} result ${result} runs ${runs} fours ${fours} sixes ${sixes} sr ${sr}`);
                console.log("''''''''''''''''''''''''''''''''''''''''''''");
